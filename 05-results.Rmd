...
ggplot(top15, aes(x = winning_rate, reorder(id, winning_rate))) +
  geom_point(size = 1) + 
  labs(x = "winning rate", y = "id", title = "Top 15 players in each level", caption = "Source: https://www.kaggle.com/datasnaek/chess") +
  theme_bw() +
...
openings <- clean_games %>%
  group_by(opening_name) %>% summarize(n = n())
  
ggplot(openings, aes(n, reorder(opening_name, n)) +
  geom_bar() +
